...
def build_charts_double_row(f1, decks, strategy, total, option):
    ...
    model_double = utility.load_json_file(models_path + decks + "/" + strategy + "-double-" + option + "-" + str(total) + ".json")
    model_stand = utility.load_json_file(models_path + decks + "/" + strategy + "-stand-" + option + "-" + str(total) + ".json")
    model_hit = utility.load_json_file(models_path + decks + "/" + strategy + "-hit-" + option + "-" + str(total) + ".json")
    if total == 12:
        model_split = utility.load_json_file(models_path + decks + "/" + strategy + "-pair-split-" + "A" + ".json")
    else:
        model_split = None
...
def build_charts_pair_split_row(f1, strategy, decks, pair):
    ...
    if pair > 8:
        total = 20
    model_split = utility.load_json_file(models_path + decks + "/" + strategy + "-pair-split-" + constant.pairs[pair] + ".json")
    if pair == 12:  # aces
        model_stand = utility.load_json_file(models_path + decks + "/" + strategy + "-stand-soft-" + str(12) + ".json")
        model_hit = utility.load_json_file(models_path + decks + "/" + strategy + "-hit-soft-" + str(12) + ".json")
    else:
        model_stand = utility.load_json_file(models_path + decks + "/" + strategy + "-stand-hard-" + str(total) + ".json")
        model_hit = utility.load_json_file(models_path + decks + "/" + strategy + "-hit-hard-" + str(total) + ".json")
...
def open_chart(decks, strategy):
    charts_path = constant.resources_url + "/charts/"
    file = open(charts_path + decks + "-" + strategy + ".json", "w")
    file.write("{\n")
